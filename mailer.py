...
def _build_code_message(to_email, code, sender):
    msg = EmailMessage()
    msg["Subject"] = f"[NETFLIX] 로그인 코드 {code}"
    msg["From"] = formataddr(("NETFLIX", sender))
    msg["To"] = to_email
    msg.set_content(
        f"로그인 인증 코드는 {code} 입니다. 5분간 유효합니다.\n\n"
        "본인이 요청하지 않았다면 이 메일은 무시하셔도 됩니다."
    )
    msg.add_alternative(
        f"""\
<div style="max-width:480px;margin:0 auto;font-family:'Malgun Gothic',sans-serif;color:#000">
  <p style="color:#e50914;font-size:26px;font-weight:800;letter-spacing:1px;margin:0 0 28px">NETFLIX</p>
  <h1 style="font-size:22px;margin:0 0 12px">로그인 코드입니다</h1>
  <p style="font-size:15px;line-height:1.6;color:#333;margin:0 0 24px">
    로그인 화면에 아래 4자리를 입력하세요. <strong>5분간</strong> 유효합니다.
  </p>
  <p style="font-size:38px;font-weight:800;letter-spacing:12px;margin:0 0 28px">{code}</p>
  <p style="font-size:12px;line-height:1.6;color:#777;margin:0">
    본인이 요청하지 않았다면 이 메일은 무시하셔도 됩니다.
    누군가 회원님의 이메일로 로그인을 시도했을 수 있습니다.
  </p>
</div>""",
        subtype="html",
    )
...
def _deliver(config, to_email, label, payload, build):
    """공통 발송 경로.

    console 백엔드면 터미널에 payload 만 찍고 끝낸다. smtp 면 build() 로
    메일을 만들어 실제로 보낸다.
    """
    ...
    if backend != config["MAIL_BACKEND"]:
        print(
            ...
            "메일을 보내지 않고 내용만 출력합니다. (.env 확인)",
            flush=True,
        )

    if backend != "smtp":
        print("\n" + "=" * 68)
        print(f"  [{label}] {to_email}")
        print(f"  {payload}")
        print("=" * 68 + "\n", flush=True)
        return
    ...
    with smtplib.SMTP(
        ...
        smtp.send_message(build(config["MAIL_SENDER"]))
...
def send_magic_link(config, to_email, link):
    ...
    _deliver(
        ...
        "매직링크",
        link,
        lambda sender: _build_message(to_email, link, sender),
    )


def send_login_code(config, to_email, code):
    """[로그인] 4자리 인증 코드."""
    _deliver(
        config,
        to_email,
        "로그인 코드",
        code,
        lambda sender: _build_code_message(to_email, code, sender),
    )
